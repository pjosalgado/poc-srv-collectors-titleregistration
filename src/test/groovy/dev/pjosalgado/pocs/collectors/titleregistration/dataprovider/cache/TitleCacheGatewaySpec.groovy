...
class TitleCacheGatewaySpec extends Specification {
    ...
    def gateway = new TitleCacheGateway(cacheProperties, redisTemplate)
    ...
    def "get returns cached enrichment data"() {
        ...
        1 * valueOperations.get("title:enrichment:Princess Mononoke") >> enrichmentData
        result.isPresent()
    ...
    def "put stores enrichment data with TTL"() {
        ...
        1 * redisTemplate.opsForValue() >> valueOperations
        1 * valueOperations.set("title:enrichment:Princess Mononoke", enrichmentData, Duration.ofHours(24))
    }
}
